...
def run_surface_smoke(
    *,
    ...
    runtime_fixture_code = textwrap.dedent(
        ...
        from codex_wake.signals import EvaluationLimits, Matched, Resume, WakeIntent
        ...
        with tempfile.TemporaryDirectory(prefix="codex-wake-runtime-fixture-") as tmp:
            root = Path(tmp) / "wake"
            uid = os.geteuid()
            identity = {"boot_id": "01234567-89ab-cdef-0123-456789abcdef", "pid": 321,
                        "start_time_ticks": 654, "owner_uid": uid}
            process_state = {"value": ProcessExitSample.alive(**identity)}
            ...
            process_adapter = ProcessExitAdapter(process_descriptor, process_registry,
                                                  lambda: process_state["value"], lambda: uid)

            class FixtureManager:
                def __init__(self):
                    self.state = "inactive"
                def resolve_unit(self, unit, *, timeout_seconds):
                    return unit
                def read_unit(self, unit, *, timeout_seconds):
                    return SystemdUnitState(unit, self.state, "fixture-manager")

            manager = FixtureManager()
            systemd_config = SystemdSourceConfig("fixture-state", "build.service", uid,
                                                 frozenset({"active", "inactive", "failed"}))
            systemd_descriptor_registry = RuntimeSourceRegistry(
                {"systemd.unit": lambda candidate: candidate == systemd_config.descriptor("active")})
            systemd_adapter = SystemdSignalAdapter(
                systemd_config, manager, systemd_descriptor_registry, SystemdReadCapability("user", uid))
            ...
            process_result = wake.register(
                WakeIntent(process_adapter.request(), Resume("fixture process", Path(tmp), {"transport": "tmux"})),
                idempotency_key="installed-process-fixture")
            systemd_result = wake.register(
                WakeIntent(systemd_adapter.request("active"), Resume("fixture systemd", Path(tmp), {"transport": "tmux"})),
                idempotency_key="installed-systemd-fixture")
            process_armed = runtime.load_armed_signal(process_result.wake_id)
            systemd_armed = runtime.load_armed_signal(systemd_result.wake_id)
            if process_armed is None or systemd_armed is None:
                raise SystemExit("fixture arms were not durable")

            # Reconstruct every adapter from the durable arm, as a fresh daemon does.
            process_restored = ProcessExitAdapter.restore(process_armed, process_registry,
                                                           lambda: process_state["value"], lambda: uid)
            systemd_restored = SystemdSignalAdapter(
                systemd_config, manager, systemd_descriptor_registry, SystemdReadCapability("user", uid))
            process_restored.runner((process_armed,)).reconcile(runtime, now, EvaluationLimits(8))
            manager.state = "active"
            SystemdSignalRunner((systemd_restored,), armed_signals=(systemd_armed,)).reconcile(
                runtime, now, EvaluationLimits(8))
            process_state["value"] = ProcessExitSample.zombie(**identity)
            process_restored.runner((process_armed,)).reconcile(runtime, now + timedelta(seconds=1), EvaluationLimits(8))
            process_match = runtime.evaluate(process_armed.wake_id, process_armed, now, EvaluationLimits(8))
            systemd_match = runtime.evaluate(systemd_armed.wake_id, systemd_armed, now, EvaluationLimits(8))
            process_repeat = runtime.evaluate(process_armed.wake_id, process_armed, now, EvaluationLimits(8))
            systemd_repeat = runtime.evaluate(systemd_armed.wake_id, systemd_armed, now, EvaluationLimits(8))
            if (not isinstance(process_match, Matched) or not isinstance(systemd_match, Matched)
                ...
                raise SystemExit("fixture runtime transitions did not match")
            for wake_id in (str(process_armed.wake_id), str(systemd_armed.wake_id)):
                cancel_record(root, wake_id, now=now)
                archive_record(root, wake_id, now=now)
            cleanup = cleanup_archived_records(root, older_than=timedelta(seconds=1),
                                               now=now + timedelta(hours=1), delete=True)
            if {item.wake_id for item in cleanup} != {str(process_armed.wake_id), str(systemd_armed.wake_id)}:
            ...
            print(json.dumps({"status": "matched", "sources": ["process.exit", "systemd.unit"],
                              "wake_ids": [str(process_armed.wake_id), str(systemd_armed.wake_id)],
