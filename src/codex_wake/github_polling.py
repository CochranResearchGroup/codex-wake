...
There is deliberately no HTTP client or credential loading here. A future
transport must prove the RunPage coverage contract; an exhausted page alone
does not prove retained history is complete. Terminal attempt evidence must
come from an authoritative attempt lookup, including its completion time.
"""
...
class GitHubReadError(Exception):
    ...
    def __init__(self, kind: str, *, retry_at: datetime | None = None):
        ...
        self.kind = kind if kind in {"auth", "rate_limit", "unavailable"} else "unavailable"
...
class GitHubPollingAdapter:
    def __init__(self, config: GitHubPollingConfig, client: GitHubReadClient, *, previous_failure: Degraded | None = None):
        ...
        if previous_failure is not None and (
            ...
            or previous_failure.code not in {"GITHUB_AUTH_UNAVAILABLE", "GITHUB_RATE_LIMITED", "GITHUB_SOURCE_UNAVAILABLE", "GITHUB_VERIFICATION_FAILED"}
    ...
    def establish_anchor(self, spec: SignalRequest, now: datetime) -> SourceAnchor | Invalid:
        if not self._valid_request(spec) or not _aware(now):
            return Invalid(None, "GITHUB_SIGNAL_NOT_ALLOWED", ("GitHub signal is outside the configured allowlist",))
        return SourceAnchor(0, self._cursor(now), {"completed_at_us": _order(now)}, "source_replay")
    ...
    def _observe(self, anchor: SourceAnchor, *, checkpoint: SourceCommit | None, now: datetime) -> PollBatch | Degraded | Invalid:
        try:
            ...
            if (anchor.recovery != "source_replay" or type(anchor.local_after_sequence) is not int
                    or anchor.local_after_sequence < 0 or set(anchor.baseline) != {"completed_at_us"}
        ...
        try:
            ...
            for _ in range(self.config.max_pages):
                ...
                for candidate in result.runs:
                    if not _valid_run(candidate):
                        return Degraded(None, "GITHUB_RESPONSE_INVALID", None)
                    if not self._allowed_run(candidate) or candidate.completed_at is None:
                        continue
            ...
            code = {"auth": "GITHUB_AUTH_UNAVAILABLE", "rate_limit": "GITHUB_RATE_LIMITED", "unavailable": "GITHUB_SOURCE_UNAVAILABLE"}[error.kind]
    ...
    def _normalize(self, run: WorkflowRun) -> NormalizedObservation:
        return NormalizedObservation(
            ...
            MappingProxyType({"workflow_id": run.workflow_id, "ref": run.ref, "conclusion": run.conclusion,
                              "run_id": run.run_id, "run_attempt": run.run_attempt, "head_sha": run.head_sha,
                              "completed_at_us": _order(run.completed_at)}),
...
def _valid_run(run: WorkflowRun) -> bool:
    try:
        return bool(
            ...
            and (run.status != "completed" or (run.conclusion is not None and run.completed_at is not None))
        )
    except (AttributeError, TypeError, ValueError):
        return False
